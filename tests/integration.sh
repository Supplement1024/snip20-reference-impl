...
function wait_for_tx() {
    ...
    if quiet jq -e '.code != null and .code != 0' <<<"$result"; then
        log "Transaction failed with code: $(jq -r '.code' <<<"$result")"
        log "Raw log: $(jq -r '.raw_log' <<<"$result")"
        return 1
    fi
...
function compute_execute() {
    local start_time end_time duration tx_hash result gas_used
    tx_hash=$(tx_of secretcli tx compute execute "$@")
    start_time=$(date +%s%3N)
    result=$(wait_for_tx "$tx_hash" "waiting for compute_execute")
    end_time=$(date +%s%3N)
    duration=$((end_time - start_time))
    gas_used=$(echo "$result" | jq -r '.gas_used')
    log "[TX] compute_execute: $tx_hash, wait+confirm: ${duration}ms, gas_used: $gas_used"
    echo "$tx_hash"
}
...
function upload_code() {
    ...
    tx_result="$(wait_for_tx "$tx_hash" 'waiting for contract upload')"
    
    # Verify logs exist before trying to extract code_id
    if quiet jq -e '.logs == null' <<<"$tx_result"; then
        log "ERROR: Contract upload transaction returned no logs"
        log "Full response: $(echo "$tx_result" | jq '.')"
        return 1
    fi
    ...
    if [ -z "$code_id" ]; then
        log "ERROR: Could not extract code_id from transaction logs"
        log "Full response: $(echo "$tx_result" | jq '.')"
        return 1
    fi
...
function instantiate() {
    ...
    local tx_hash start_time end_time duration result gas_used contract_address
    tx_hash="$(tx_of secretcli tx compute instantiate "$code_id" "$init_msg" --label "$(label_by_id "$code_id")" ${FROM[a]} --gas 10000000)"
    start_time=$(date +%s%3N)
    result=$(wait_for_tx "$tx_hash" 'waiting for init to complete')
    end_time=$(date +%s%3N)
    duration=$((end_time - start_time))
    gas_used=$(echo "$result" | jq -r '.gas_used')
    log "[TX] instantiate: $tx_hash, wait+confirm: ${duration}ms, gas_used: $gas_used"
    contract_address=$(echo "$result" | jq -r '.logs[0].events[0].attributes[] | select(.key == "contract_address") | .value')
...
function create_contract() {
    ...
    # instantiate 现在直接返回合约地址
    local contract_address
    contract_address="$(instantiate "$code_id" "$init_msg")"
    echo "$contract_address"
}
...
function deposit() {
    ...
    local start_time end_time duration result gas_used
    tx_hash="$(tx_of secretcli tx compute execute "$contract_addr" "$deposit_message" --amount "${amount}uscrt" ${FROM[$key]} --gas 250000)"
    start_time=$(date +%s%3N)
    result=$(wait_for_tx "$tx_hash" "waiting for deposit to \"$key\" to process")
    end_time=$(date +%s%3N)
    duration=$((end_time - start_time))
    gas_used=$(echo "$result" | jq -r '.gas_used')
    log "[TX] deposit: $tx_hash, wait+confirm: ${duration}ms, gas_used: $gas_used"
    deposit_response=$(echo "$result" | jq -ec '.answers[0].output_data_as_string' | sed 's/\\//g' | sed 's/ //g' | sed 's/^"\(.*\)"$/\1/')
...
function mint() {
    ...
    local deposit_response
    local start_time end_time duration result gas_used
    tx_hash="$(tx_of secretcli tx compute execute "$contract_addr" "$mint_message" ${FROM[$key]} --gas 251000)"
    start_time=$(date +%s%3N)
    result=$(wait_for_tx "$tx_hash" "waiting for mint to \"$recipient\" to process")
    end_time=$(date +%s%3N)
    duration=$((end_time - start_time))
    gas_used=$(echo "$result" | jq -r '.gas_used')
    log "[TX] mint: $tx_hash, wait+confirm: ${duration}ms, gas_used: $gas_used"
    echo "$tx_hash"
    deposit_response=$(echo "$result" | jq -ec '.answers[0].output_data_as_string' | sed 's/\\//g' | sed 's/ //g' | sed 's/^"\(.*\)"$/\1/')
...
function burn() {
    ...
    local burn_response
    local start_time end_time duration result gas_used
    tx_hash="$(tx_of secretcli tx compute execute "$contract_addr" "$burn_message" ${FROM[$key]} --gas 250000)"
    start_time=$(date +%s%3N)
    result=$(wait_for_tx "$tx_hash" "waiting for burn for \"$key\" to process")
    end_time=$(date +%s%3N)
    duration=$((end_time - start_time))
    gas_used=$(echo "$result" | jq -r '.gas_used')
    log "[TX] burn: $tx_hash, wait+confirm: ${duration}ms, gas_used: $gas_used"
    echo "$tx_hash"
    burn_response=$(echo "$result" | jq -ec '.answers[0].output_data_as_string' | sed 's/\\//g' | sed 's/ //g' | sed 's/^"\(.*\)"$/\1/')
    log "$burn_response"
...
function main() {
    ...
    if ! secretcli ls /root/code/contract.wasm.gz 2>/dev/null | grep -q contract.wasm.gz; then
        log "ERROR: Contract file not found in container. Checking if file exists locally..."
        if [ ! -f "contract.wasm.gz" ]; then
            log "ERROR: contract.wasm.gz not found in current directory!"
            log "Please run 'make compile-optimized' first"
            return 1
        fi
    fi

    # Wait for Docker container to be fully initialized
    local max_attempts=30
    local attempt=0
    until secretcli status > /dev/null 2>&1; do
        attempt=$((attempt + 1))
        if [ $attempt -ge $max_attempts ]; then
            log "ERROR: Docker container failed to initialize after $max_attempts attempts"
            return 1
        fi
        log "Waiting for container to initialize... (attempt $attempt/$max_attempts)"
        sleep 2
    done
